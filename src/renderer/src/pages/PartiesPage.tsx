...
import { Plus, BookOpen, Pencil, Archive, Search, ChevronLeft, ChevronRight, TrendingDown, TrendingUp, Users } from 'lucide-react'
...
interface PagedResult { data: (Party & { current_balance: number; last_entry_date: string | null })[]; total: number; page: number; limit: number }
...
export default function PartiesPage(): React.ReactElement {
  ...
  const fetchPage = useCallback(async (p: number, q: string, t: string) => {
    ...
    try {
      ...
      showToast('error', getApiError(err, 'Failed to load parties'))
    } finally {
      setLoading(false)
    }
  }, [get, set, showToast])
  ...
  const handleSave = async (): Promise<void> => {
    if (!form.name.trim()) { showToast('error', 'Party name is required'); return }
    setSaving(true)
    try {
      if (editParty) {
        await apiClient.patch(`/invoice/parties/${editParty.id}`, form)
        showToast('success', 'Party updated')
      } else {
  ...
  const handleArchive = async (): Promise<void> => {
    ...
    try {
      ...
      showToast('success', 'Party archived')
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="flex items-center justify-between px-6 py-3 bg-white border-b border-border flex-shrink-0">
        <h1 className="text-xl font-bold text-text-primary">Parties</h1>
        <Button onClick={openAdd} size="sm"><Plus size={14} /> Add Party</Button>
      </div>
        ...
        {statsLoading ? (
          ...
          <>
            <div className="bg-bg-card rounded-lg border border-border border-l-4 border-l-red-500 px-4 py-2.5 flex items-center gap-3 flex-1">
              ...
              <div>
                <div className="text-xs text-text-secondary">Total Receivable (they owe you)</div>
            ...
            <div className="bg-bg-card rounded-lg border border-border border-l-4 border-l-green-500 px-4 py-2.5 flex items-center gap-3 flex-1">
              ...
              <div>
                <div className="text-xs text-text-secondary">Total Payable (you owe them)</div>
            ...
            <div className="bg-bg-card rounded-lg border border-border border-l-4 border-l-accent px-4 py-2.5 flex items-center gap-3 flex-1">
              ...
              <div>
                <div className="text-xs text-text-secondary">Total Parties</div>
        ...
        <div className="bg-bg-card rounded-lg border border-border overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b-2 border-border">
              <tr>
                {['Party Name', 'Type', 'Phone', 'Balance', 'Last Entry', 'Actions'].map((h) => (
            ...
            <tbody>
              ...
              {!loading && parties.length === 0 && (
                <tr><td colSpan={6} className="text-center text-text-secondary py-10">
                  {search ? `No parties match "${search}"` : 'No parties yet. Click "Add Party" to add one.'}
                </td></tr>
              )}
                  ...
                  <tr key={p.id} className="border-b border-border hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2.5">
                      <div className="flex items-center gap-1">
                        <Button variant="ghost" size="sm" onClick={() => navigate(`/parties/${p.id}/ledger`)}><BookOpen size={13} /> Ledger</Button>
      ...
      <div className="flex-shrink-0 border-t border-border bg-white px-6 py-2 flex items-center justify-between text-sm text-text-secondary">
        <span>{loading ? 'Loading…' : total === 0 ? 'No parties' : `Showing ${(page - 1) * PAGE_SIZE + 1}–${Math.min(page * PAGE_SIZE, total)} of ${total}`}</span>
        {totalPages > 1 && (
          <div className="flex items-center gap-1">
      ...
      {/* Add/Edit Modal */}
      <Modal open={modalOpen} title={editParty ? 'Edit Party' : 'Add Party'} onClose={() => setModalOpen(false)}
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-text-primary mb-2">Party Type *</label>
      ...
      {/* Archive Confirm */}
      <Modal open={!!archiveConfirm} title="Archive Party" onClose={() => setArchiveConfirm(null)}
